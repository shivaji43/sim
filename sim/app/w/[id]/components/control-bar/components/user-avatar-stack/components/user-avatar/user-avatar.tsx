...
export function UserAvatar({
  ...
  // If tooltipContent is explicitly null, don't show tooltip
  if (tooltipContent === null) {
    return (
      <div
        className={`relative ${sizeClass} rounded-full flex items-center justify-center text-white font-semibold cursor-pointer`}
        style={avatarStyle}
      >
        {initials}
      </div>
  ...
  return (
    <Tooltip>
      <TooltipTrigger>
        <div
          className={`relative ${sizeClass} rounded-full flex items-center justify-center text-white font-semibold transition-transform hover:scale-110 cursor-pointer`}
          style={avatarStyle}
        >
          {initials}
        </div>
      ...
      <TooltipContent side="bottom" align="center" className="text-xs">
        {tooltipContent || `User ${connectionId}`}
      </TooltipContent>
    </Tooltip>
  )
}
